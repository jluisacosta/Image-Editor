...
class tabla_base():
	...
	def __init__(self,histograma,nt):
		self.items = []
		self.lista_tablas = []
		self.orden_simbolos = []
		self.parte1 = []
		...
		self.items.sort()
		self.parte1 = self.items[:nt]
		parte2 = self.items[nt:]
...
def genera_tabla_codigos_truncado(histograma,nt):
	t = tabla_base(histograma,nt)
	...
	for i in t.items:
		if i.codigo == "":
			i.codigo = codigo+genera_cadena01(cont,tam)
			cont += 1
		else:
			tam = len(i.codigo)-len(codigo)+1
